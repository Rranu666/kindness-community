...
export default function VisionMission() {
  ...
  return (
    <section id="vision" className="bg-slate-50" ref={ref}>
      ...
      {/* ── MISSION (unchanged structure, light background) ─────────────── */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#0d1b2a] leading-tight mb-6">
          ...
          >
            {missionItems.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-5 p-5 rounded-2xl bg-white border border-slate-100 hover:border-rose-200 hover:shadow-sm transition-all duration-300"
              >
                ...
                <span className="text-[#0d1b2a] font-semibold">{item.text}</span>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      {/* Kindness Constitution */}
      <div className="bg-[#0d1b2a] mt-0 overflow-hidden">
        <div className="max-w-5xl mx-auto px-6 lg:px-12 py-16">
          ...
          >
            ...
            <p className="text-white/50 text-sm uppercase tracking-widest font-medium mb-8">A Guiding Charter for the Kindness Community</p>
          </motion.div>
          ...
          >
            <h3 className="text-rose-400 text-xs font-bold tracking-widest uppercase mb-4">Preamble</h3>
            <div className="space-y-3 text-white/70 text-sm leading-relaxed">
              ...
              <p className="text-white font-semibold pt-2">Our purpose is simple: To help create a Haven on Earth through kindness in action.</p>
            </div>
          </motion.div>
          ...
          >
            <h3 className="text-white font-bold text-xl mb-6 text-center">The Twelve Traditions of the Kindness Community</h3>
            <div className="grid sm:grid-cols-2 gap-4">
              {(expanded ? traditions : traditions.slice(0, 6)).map((t, i) => (
                <motion.div
                  key={t.num}
                  initial={{ opacity: 0, y: 16 }}
                  animate={inView ? { opacity: 1, y: 0 } : {}}
                  transition={{ duration: 0.4, delay: 0.5 + i * 0.06 }}
                  className="flex gap-4 p-5 rounded-xl bg-white/5 border border-white/10 hover:border-rose-500/40 hover:bg-white/8 transition-all duration-300"
                >
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-pink-500 flex items-center justify-center flex-shrink-0 text-white text-xs font-bold shadow-md shadow-rose-900/50">
                    {t.num}
                  </div>
                  <div>
                    <div className="text-white font-semibold text-sm mb-1">{t.title}</div>
                    <div className="text-white/55 text-xs leading-relaxed">{t.text}</div>
                  </div>
                </motion.div>
              ))}
            </div>
            <div className="text-center mt-6">
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/15 border border-white/20 text-white/70 hover:text-white text-sm font-medium transition-all duration-200"
              >
                {expanded ? <><ChevronUp className="w-4 h-4" /> Show Less</> : <><ChevronDown className="w-4 h-4" /> View All 12 Traditions</>}
              </button>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="mt-10 border-l-4 border-rose-500 pl-6 py-2"
          >
            ...
            <p className="text-white font-semibold text-sm">Together, we work toward a Haven on Earth.</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="mt-12"
          >
            <h3 className="text-white font-bold text-xl mb-2 text-center">How the Kindness Ecosystem Works</h3>
            <p className="text-white/50 text-sm text-center mb-8">The Kindness Constitution provides the guiding principles for the entire ecosystem</p>
            <div className="flex flex-col items-center gap-0">
              {ecosystemSteps.map((step, i) => (
                <div key={i} className="flex flex-col items-center w-full max-w-lg">
                  <div className={`w-full px-6 py-4 rounded-xl text-center text-sm font-medium border transition-all duration-300 ${i === 0 ? 'bg-gradient-to-r from-rose-500 to-pink-500 text-white border-rose-400 shadow-lg shadow-rose-900/40' : 'bg-white/5 border-white/10 text-white/70'}`}>
                    {step}
                  </div>
            ...
            <p className="text-white/40 text-xs text-center mt-8 leading-relaxed max-w-xl mx-auto">
              This charter is intentionally simple. Its purpose is to ensure that as the community grows&#8212;across organizations, platforms, and technologies&#8212;we remain aligned with the values that inspired it: kindness, service, unity, and shared human progress.
            </p>
          </motion.div>
        </div>
      </div>
