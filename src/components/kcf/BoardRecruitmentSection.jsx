...
export default function BoardRecruitmentSection() {
  ...
  return (
    <section id="board" className="py-24 lg:py-32 bg-white relative overflow-hidden" ref={ref}>
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-rose-50 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0d1b2a] leading-tight mb-4">
            We recruit{" "}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-14">
          {idealProfiles.map((profile, i) => (
            <motion.div
              ...
              className="p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:border-rose-200 hover:shadow-md hover:shadow-rose-50 transition-all duration-300"
            >
              ...
              <h3 className="font-bold text-[#0d1b2a] text-sm mb-2">{profile.title}</h3>
              <p className="text-slate-500 text-xs leading-relaxed">{profile.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          >
            <p className="text-slate-400 text-xs font-bold tracking-widest uppercase mb-5">Board Commitment Expectations</p>
            <div className="space-y-3">
              {commitments.map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className="text-[#0d1b2a] text-sm font-medium">{item.text}</span>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        >
          <p className="text-slate-400 text-sm mb-4 italic">"This is governance-first. We build trust before we build scale."</p>
          <a
            href="mailto:contact@kindnesscommunityfoundation.com?subject=Board Membership Inquiry"
            className="inline-flex items-center gap-2 px-7 py-3 bg-[#0d1b2a] hover:bg-slate-800 text-white text-sm font-semibold rounded-xl transition-colors"
          >
